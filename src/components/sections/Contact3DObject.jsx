...
const SOCIAL_ORBITS = {
  instagram: { tiltZ: SCENE_CONFIG.orbit.tiltA, speed: 0.27, radius: SCENE_CONFIG.orbit.radius, initialPhase: 0 },
  linkedin: { tiltZ: SCENE_CONFIG.orbit.tiltA, speed: 0.27, radius: SCENE_CONFIG.orbit.radius, initialPhase: Math.PI },
  github: { tiltZ: SCENE_CONFIG.orbit.tiltB, speed: -0.27, radius: SCENE_CONFIG.orbit.radius, initialPhase: Math.PI / 2 },
  leetcode: { tiltZ: SCENE_CONFIG.orbit.tiltB, speed: -0.27, radius: SCENE_CONFIG.orbit.radius, initialPhase: -Math.PI / 2 },
}
...
function OrbitingIcon({ social, tiltZ, speed, radius, initialPhase, index, distanceFactor, hoveredCountRef }) {
  ...
  useFrame((state, delta) => {
    ...
    if (rotationRef.current) {
      rotationRef.current.rotation.y += delta * speed * speedMultiplier.current * f
    }
  ...
  return (
    <group rotation={[SCENE_CONFIG.orbit.tiltX, 0, tiltZ]}>
  ...
  return (
    <group scale={scale}>
      ...
      {socialsToRender.map((social, i) => {
        const config = SOCIAL_ORBITS[social.platform]
        return (
          <OrbitingIcon
            key={social.platform}
            social={social}
